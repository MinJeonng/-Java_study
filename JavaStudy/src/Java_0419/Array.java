...
public class Array {
	// 가변적 매개변수
	public static int addNumber(int... nums) { // 배열의 개수를 정하지 않고, 그거에 상관없이 어떤 값이든 + 하게 만들어 줌, 그리고 스프레드는 매개변수 중에서 항상 맨 뒤에
												// 잇어야한다.
		int total = 0;
		for (int num : nums) {
			total += num;
	...
	public static void main(String[] args) {
		...
		// 2차원 배열
		// 선언
		int[][] matrix = { { 1, 2, 3 }, { 100, 200, 300 }, { 8, 8, 10 } };
		// 첫번째 출력
		for (int[] row : matrix) {
		...
		// 두번째 출력방법
		for (int i = 0; i < matrix.length; i++) { // 1행에 [0,1] [0,2] 이렇게 돌아가서 배열이 출력됌
			for (int j = 0; j < matrix[0].length; j++) { // 여기서 1,2 이렇게 적용된다.
